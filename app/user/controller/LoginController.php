...
//            }
            ...
            $userid = $user ? $user['c_userid'] : get_userid($data['username']);
            if (!$userid) $this->error('您的用户名/密码与任何现有的帐户都不匹配，请重试。');
            clean_cookie($userid);
            $loginUrl = get_api("loginPath");
            $cookieFile = CMF_ROOT . "data/cookie/".$userid;
            $param = array('j_username'=>$data['username'], 'j_password'=>$data['password']);
            login_post($loginUrl, $cookieFile, $param);
            if (file_get_contents($cookieFile)) {
                if (!$user) {
                    $user['user_email'] = $data['username'];
                    $user['user_pass'] = $data['password'];
                    $user['userid'] = $userid;
                    $userModel->registerEmail($user);
                } else {
                    $user['user_email'] = $data['username'];
                    $user['user_pass'] = $data['password'];
                    $userModel->doEmail($user);
                }
                $session_login_http_referer = session('login_http_referer');
                $redirect = empty($session_login_http_referer) ? $this->request->root() : $session_login_http_referer;
                cmf_user_action('login');
                ...
                $this->error('您的用户名/密码与任何现有的帐户都不匹配，请重试。');
            }
        } else {
            $this->error("请求错误");
        }
